...
object Day2 extends IOApp {
  ...
  object OpCode {
    ...
    final case class Add(left: Pointer, right: Pointer, dest: Pointer)
        extends OpCode
    final case class Mul(left: Pointer, right: Pointer, dest: Pointer)
        extends OpCode
    ...
    nextOp match {
      ...
      case Add(l, r, dest) =>
        update(machine.mem, dest, l, r, _ + _) match {
          ...
          case Some(newMem) =>
            Right(machine.copy(pc = machine.pc + 4, mem = newMem))
        }
      case Mul(l, r, dest) =>
        update(machine.mem, dest, l, r, _ * _) match {
          case None => Left(machine)
  ...
  def run(init: Machine): Either[Machine, Machine] = {
    def loop(state: Either[Machine, Machine]): Either[Machine, Machine] =
      state match {
